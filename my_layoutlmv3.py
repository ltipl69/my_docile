...
LILT_DOWNSTREAM_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `({0})`):
            Indices of input sequence tokens in the vocabulary.
...
class MyLiltConfig(PretrainedConfig):
    r"""
...
class MyLiltPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = MyLiltConfig
    base_model_prefix = "lilt"
    # base_model_prefix = "SCUT-DLVCLab/lilt-roberta-en-base"
...
class MyLiltClassificationHead(nn.Module):
    ...
    def forward(self, features, **kwargs):
        ...
        x = self.out_proj(x)
        return x
class MyLiltForTokenClassification(MyLiltPreTrainedModel):
