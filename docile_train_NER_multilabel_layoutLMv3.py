...
from my_layoutlmv3 import MyLiltForTokenClassification
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        "SCUT-DLVCLab/lilt-roberta-en-base", add_prefix_space=True if args.model_name == "roberta-base" else False
    )
    ...
    if args.pretrained_weights:
        if args.pretrained_weights.is_dir():
            ...
            model = MyLiltForTokenClassification(config=config)
            model.load_state_dict(model_weights["state_dict"], strict=False)
    else:
        model = MyLiltForTokenClassification.from_pretrained("SCUT-DLVCLab/lilt-roberta-en-base", config=config)
