...
OUTPUT_DIR_PREFIX="/content/docile/output/trainings"

NER_SCRIPTS_DIR="/content/docile/baselines/NER"
CHECKPOINTS_DIR="/content/docile/baselines/checkpoints"

# Common parameters for all trainings with exception of roberta_pretraining
DATA="--docile_path /content/docile/data"
USE_PREPROCESSED="--preprocessed_dataset_path /content/drive/MyDrive/newpreprocessed_dataset"
OTHER_COMMON_PARAMS="--save_total_limit 1 --weight_decay 0.001 --lr 2e-5 --dataloader_num_workers 32 --use_BIO_format --tag_everything --report_all_metrics --stride 0"
...
function run_training() {
  ...
  training_cmd="CUDA_LAUNCH_BLOCKING=1 TF_FORCE_GPU_ALLOW_GROWTH=\"true\" TORCH_USE_CUDA_DSA=true CUDA_VISIBLE_DEVICES=${GPU} poetry run python3 ${NER_SCRIPTS_DIR}/${cmd} ${params} 2>&1 | tee ${log}"
...
if [[ " ${run} " =~ " ${single_run} " ]]; then
  train_params="--train_bs 1 --test_bs 1 --num_epochs 1500 --gradient_accumulation_steps 1 --warmup_ratio 0"
  model="--model_name nielsr/lilt-xlm-roberta-base --use_roberta"
...
if [[ " ${run} " =~ " ${single_run} " ]]; then
  train_params="--train_bs 1 --test_bs 1 --num_epochs 20 --gradient_accumulation_steps 1 --warmup_ratio 0"
  model="--model_name SCUT-DLVCLab/lilt-roberta-en-base --use_lilt"
  all_params="${COMMON_PARAMS} ${train_params} ${model}"
  output_dir="${single_run}/${TIMESTAMP}"
  run_training ${CMD_ROBERTA} ${output_dir} ${all_params}
fi
...
if [[ " ${run} " =~ " ${single_run} " ]]; then
  train_params="--train_bs 1 --test_bs 1 --num_epochs 200 --gradient_accumulation_steps 1 --warmup_ratio 0"
  model="--model_name ${CHECKPOINTS_DIR}/roberta_ours_133352 --use_roberta"
  all_params="${COMMON_PARAMS} ${train_params} ${model}"
  output_dir="${single_run}/${TIMESTAMP}"
  run_training ${CMD_ROBERTA} ${output_dir} ${all_params}
fi
...
if [[ " ${run} " =~ " ${single_run} " ]]; then
  ...
  model="--model_name microsoft/layoutlmv3-base --pretrained_weights ${CHECKPOINTS_DIR}/layoutlmv3_ours_320587"
  all_params="${COMMON_PARAMS} ${train_params} ${model}"
  output_dir="${single_run}/${TIMESTAMP}"
  run_training ${CMD_LAYOUTLMV3} ${output_dir} ${all_params}
fi
